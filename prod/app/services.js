angular.module('EMLMaker').factory(
  "$AppTour", function $AppTour(){


    var self = this;
    this.getKeyInRegistry = function(key) {
      if(window.localStorage) {
        var original = JSON.parse(localStorage.getItem("appTour"));
        if(original[key]) {
          return original[key];
        } else {
          return false;
    ...
    this.updateKeyInRegistry = function(key, value){
      if(window.localStorage) {
        var original = JSON.parse(localStorage.getItem("appTour"));
        original[key] = value;
        localStoage.setItem("appTour", JSON.stringify(original))
      } else {
        // do something else
      }
    };
    this.init = function(){
        if(window.localStorage) {
          if(localStorage.getItem("appTour") === null){
            localStorage.setItem("appTour", JSON.stringify({}));
          }
        } else {
          //console.log('localStoage is not an option');
        }
      };

    this.isFirstTime = function(key){
      if(self.getKeyInRegistry(key)){
        return false;
      } else {
        return true;
      }
